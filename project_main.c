// Team Members:
// - Yumish Niroula (merge all the code files together, detectMorseCode, SpaceDetected)
// - Vy Nguyen (uartTaskFxn, displayMessage, closeDisplayAndReopenUART, closeUARTandInitDisplay, sendCharacterViaUART, playMorseCode)
// - Niranjan Sreegith (sensorTaskFxn, initBuzzer, beep, button0Fxn, button1Fxn)
...
static PIN_Config buzzerConfig[] = {Board_BUZZER | PIN_GPIO_OUTPUT_EN | PIN_GPIO_LOW | PIN_PUSHPULL | PIN_DRVSTR_MAX,
                                    PIN_TERMINATE};
...
bool sendCharacterViaUART(const char character)
{
    ...
    if (UART_write(uartHandle, buffer, len + 1) != len)
    {
...
void SpaceDetected()
{
    System_printf("Light pressed: Adding space\n");
...
// COULD NOT GET THE DISPLAY AND UART TO WORK AT THE SAME TIME BEFORE THE DEADLINE:

void closeUARTandInitDisplay()
{
    System_printf("Starting closeUARTandInitDisplay\n");
    System_flush();

    if (uartConnected && uartHandle != NULL)
    {
        System_printf("Closing UART connection\n");
        System_flush();
        UART_close(uartHandle);
        uartConnected = false;
    }

    System_printf("Display initialized status: %d\n", displayInitialized);
    System_flush();
    ...
    if (!displayInitialized)
    {
        ...
        System_printf("Attempting to open display...\n");
        System_flush();
        ...
        {
            System_printf("ERROR: Display_open failed!\n");
            System_flush();
            return;
        }

        System_printf("Display opened successfully\n");
        System_flush();
    ...
    System_printf("Final display handle status: %p\n", displayHandle);
    System_flush();
}
